...
def visual_physics_moments(x, eps=1.0e-6):
    """Differentiable layout moments from visual/latent mass and edge mass.

    This is intentionally category-free: it asks where visual energy lives, how
    spread/oriented it is, and where edges live, without naming objects or rules.
    """
    if x.ndim != 4:
...
def visual_physics_loss(pred, target, prefix="visual_physics", eps=1.0e-6):
    if pred.shape != target.shape:
        raise ValueError(
            f"visual physics loss shapes must match, got "
            f"{tuple(pred.shape)} and {tuple(target.shape)}")
    pred_m = visual_physics_moments(pred, eps=eps)
    target_m = visual_physics_moments(target.detach(), eps=eps)
    moment_l1 = F.l1_loss(pred_m, target_m)
    mass_l1 = F.l1_loss(pred_m[:, (0, 6)], target_m[:, (0, 6)])
    centroid_l1 = F.l1_loss(
        torch.cat((pred_m[:, 1:3], pred_m[:, 7:9]), dim=-1),
        torch.cat((target_m[:, 1:3], target_m[:, 7:9]), dim=-1))
    inertia_l1 = F.l1_loss(
        torch.cat((pred_m[:, 3:6], pred_m[:, 9:12]), dim=-1),
        torch.cat((target_m[:, 3:6], target_m[:, 9:12]), dim=-1))
    return moment_l1, {
        f"{prefix}_loss": moment_l1.detach(),
        f"{prefix}_mass_l1": mass_l1.detach(),
        f"{prefix}_centroid_l1": centroid_l1.detach(),
        f"{prefix}_inertia_l1": inertia_l1.detach(),
        f"{prefix}_pred_mass_mean": pred_m[:, 0].detach().mean(),
        f"{prefix}_target_mass_mean": target_m[:, 0].detach().mean(),
        f"{prefix}_pred_edge_mass_mean": pred_m[:, 6].detach().mean(),
        f"{prefix}_target_edge_mass_mean": target_m[:, 6].detach().mean(),
    }
...
def flow_hidden_feature_structure_loss(aligner, hidden_tokens, feature_tokens,
                                       prefix="flow_repa_structure", eps=1.0e-6):
    if aligner is None:
        zero = hidden_tokens.sum() * 0.0
        return zero, {}
    feature_tokens = feature_tokens.to(device=hidden_tokens.device)
    ...
    bsz, hidden_n, _dim = hidden_emb.shape
    losses = []
    hidden_offdiag_means = []
    feature_offdiag_means = []
    for i in range(int(bsz)):
        feat_i = feat_emb[i, valid[i]]
        if int(feat_i.shape[0]) <= 1 or hidden_n <= 1:
            continue
        hidden_i = F.normalize(hidden_emb[i].float(), dim=-1, eps=float(eps))
        feat_i = _resize_token_sequence(feat_i.float(), hidden_n)
        feat_i = F.normalize(feat_i.detach(), dim=-1, eps=float(eps))
        hidden_sim = hidden_i.matmul(hidden_i.t())
        feat_sim = feat_i.matmul(feat_i.t())
        eye = torch.eye(hidden_n, dtype=torch.bool, device=hidden_sim.device)
        hidden_rel = hidden_sim.masked_select(~eye)
        feat_rel = feat_sim.masked_select(~eye)
        losses.append(F.mse_loss(hidden_rel, feat_rel))
        hidden_offdiag_means.append(hidden_rel.detach().mean())
        feature_offdiag_means.append(feat_rel.detach().mean())
    if not losses:
        zero = hidden_tokens.sum() * 0.0 + feature_tokens.sum() * 0.0
        return zero, {
            f"{prefix}_loss": zero.detach(),
            f"{prefix}_hidden_offdiag_mean": zero.detach(),
            f"{prefix}_feature_offdiag_mean": zero.detach(),
            f"{prefix}_n": torch.tensor(0.0, device=hidden_tokens.device),
            f"{prefix}_hidden_tokens": torch.tensor(float(hidden_n), device=hidden_tokens.device),
            f"{prefix}_feature_tokens": valid.float().sum(dim=1).mean().detach(),
        }
    ...
    return loss, {
        f"{prefix}_loss": loss.detach(),
        f"{prefix}_hidden_offdiag_mean": torch.stack(hidden_offdiag_means).mean(),
        f"{prefix}_feature_offdiag_mean": torch.stack(feature_offdiag_means).mean(),
        f"{prefix}_n": torch.tensor(float(len(losses)), device=hidden_tokens.device),
        f"{prefix}_hidden_tokens": torch.tensor(float(hidden_n), device=hidden_tokens.device),
        f"{prefix}_feature_tokens": valid.float().sum(dim=1).mean().detach(),
    }
...
def endpoint_statistics_axes(x):
    if x.ndim <= 1:
...
def flow_endpoint_statistics_loss(endpoint_pred, clean_target, mean_w=1.0, std_w=1.0,
                                  prefix="flow_endpoint_stats", eps=1.0e-6):
    if endpoint_pred.shape != clean_target.shape:
        raise ValueError(
            f"endpoint statistics shapes differ: pred={tuple(endpoint_pred.shape)} "
            f"target={tuple(clean_target.shape)}")
    pred = endpoint_pred.float()
    target = clean_target.float().detach()
    zero = pred.sum() * 0.0
    mean_w = float(mean_w)
    std_w = float(std_w)
    if mean_w < 0.0 or std_w < 0.0:
        raise ValueError("endpoint statistics component weights must be non-negative")
    axes = endpoint_statistics_axes(pred)
    if not axes:
        pred_mean = pred
        target_mean = target
        ...
        pred_mean = pred.mean(dim=axes)
        target_mean = target.mean(dim=axes)
        pred_std = pred.std(dim=axes, unbiased=False)
        target_std = target.std(dim=axes, unbiased=False)
    mean_loss = F.mse_loss(pred_mean, target_mean) if mean_w > 0.0 else zero
    std_loss = F.mse_loss(pred_std, target_std) if std_w > 0.0 else zero
    loss = mean_w * mean_loss + std_w * std_loss
    return loss, {
        f"{prefix}_loss": loss.detach(),
        f"{prefix}_mean_loss": mean_loss.detach(),
        f"{prefix}_std_loss": std_loss.detach(),
        f"{prefix}_pred_mean_abs": pred_mean.detach().abs().mean(),
        f"{prefix}_target_mean_abs": target_mean.detach().abs().mean(),
        f"{prefix}_pred_std_mean": pred_std.detach().mean(),
        f"{prefix}_target_std_mean": target_std.detach().mean(),
        f"{prefix}_axes": torch.tensor(float(len(axes)), device=endpoint_pred.device),
        f"{prefix}_eps": torch.tensor(float(eps), device=endpoint_pred.device),
    }
...
def latent_flow_losses(flow, z1, cond, cond_drop=0.0, ae=None,
                       ...
                       repa_mode="pooled", repa_structure_w=0.0,
    ...
    endpoint_stats_w = float(endpoint_stats_w)
    endpoint_stats_mean_w = float(endpoint_stats_mean_w)
    endpoint_stats_std_w = float(endpoint_stats_std_w)
    if endpoint_stats_w < 0.0:
        raise ValueError("endpoint_stats_w must be non-negative")
    if endpoint_stats_mean_w < 0.0 or endpoint_stats_std_w < 0.0:
        raise ValueError("endpoint statistics component weights must be non-negative")
    ...
    if physics_w < 0.0:
        raise ValueError("physics_w must be non-negative")
    ...
    repa_structure_w = float(repa_structure_w)
    if repa_structure_w < 0.0:
        raise ValueError("repa_structure_w must be non-negative")
    ...
    need_flow_features = (
        ...
        or (repa_aligner is not None and repa_structure_w > 0.0)
    ...
    parts = {
        ...
        "flow_endpoint_stats_w": torch.tensor(float(endpoint_stats_w), device=z1.device),
        "flow_endpoint_stats_mean_w": torch.tensor(
            float(endpoint_stats_mean_w), device=z1.device),
        ...
        "flow_physics_w": torch.tensor(float(physics_w), device=z1.device),
    ...
    if endpoint_stats_w > 0.0:
        endpoint_stats, endpoint_stats_parts = flow_endpoint_statistics_loss(
            ...
            mean_w=endpoint_stats_mean_w,
            std_w=endpoint_stats_std_w)
        total = total + float(endpoint_stats_w) * endpoint_stats
        parts.update(endpoint_stats_parts)
    if consistency_w > 0.0 or straightness_w > 0.0:
        ...
        if straightness_w > 0.0:
            per_sample_straightness = (
                pred.float() - pred2.float()).pow(2).flatten(1).mean(dim=1)
            straightness = (
                per_sample_straightness * velocity_weights.to(
                    per_sample_straightness.dtype)).mean()
            straightness_unweighted = per_sample_straightness.mean()
            total = total + float(straightness_w) * straightness
            parts["flow_straightness_velocity_mse"] = straightness.detach()
            parts["flow_straightness_velocity_mse_unweighted"] = (
                straightness_unweighted.detach())
            parts["flow_straightness_time_delta_mean"] = (
                (t - t2).abs().detach().mean())
            parts["flow_straightness_time_delta_min"] = (
                (t - t2).abs().detach().min())
    ...
    if repa_aligner is not None and repa_structure_w > 0.0:
        if flow_features is None or "image_tokens" not in flow_features:
            raise ValueError("flow REPA structure alignment requires transformer image-token features")
        if image_feature_tokens is None:
            raise ValueError("flow REPA structure alignment requires image_embedding_sequence rows")
        structure, structure_parts = flow_hidden_feature_structure_loss(
            repa_aligner, flow_features["image_tokens"], image_feature_tokens)
        total = total + float(repa_structure_w) * structure
        parts.update(structure_parts)
        parts["flow_repa_structure_w"] = torch.tensor(
            float(repa_structure_w), device=z1.device)
...
def load_checkpoint(path, device=DEV, prefer_ema=True):
    ...
    return ae, flow, conditioner, prompt_vocab, prompt_templates, {
        ...
        "flow_endpoint_stats_w": float(ckpt.get(
            "flow_endpoint_stats_w", report.get("flow_endpoint_stats_w", 0.0)) or 0.0),
        "flow_endpoint_stats_mean_w": float(ckpt.get(
            "flow_endpoint_stats_mean_w",
            report.get("flow_endpoint_stats_mean_w", 1.0)) or 1.0),
        "flow_endpoint_stats_std_w": float(ckpt.get(
            "flow_endpoint_stats_std_w",
        ...
        **latent_stats_report(latent_stats),
        ...
        "flow_repa_structure_w": float(ckpt.get(
            "flow_repa_structure_w", report.get("flow_repa_structure_w", 0.0)) or 0.0),
...
def train_latent_flow(ae_steps=200, flow_steps=200, batch=64, latent_ch=16, hidden=64,
    ...
    if image_manifest:
        ...
        image_embedding_in_dim = (
            ...
            if (image_feature_align_w > 0.0 or flow_feature_align_w > 0.0
                or flow_repa_w > 0.0 or flow_repa_structure_w > 0.0) else 0
        )
        if (image_feature_align_w > 0.0 or flow_feature_align_w > 0.0
        ...
        if (flow_repa_structure_w > 0.0
            ...
            if not has_image_embedding_sequences:
                raise ValueError(
                    "flow REPA token/structure alignment requires "
                    "image_embedding_sequence rows")
    else:
    ...
    flow_endpoint_stats_w = float(flow_endpoint_stats_w)
    flow_endpoint_stats_mean_w = float(flow_endpoint_stats_mean_w)
    flow_endpoint_stats_std_w = float(flow_endpoint_stats_std_w)
    if flow_endpoint_stats_w < 0.0:
        raise ValueError("flow_endpoint_stats_w must be non-negative")
    if flow_endpoint_stats_mean_w < 0.0 or flow_endpoint_stats_std_w < 0.0:
        raise ValueError("flow endpoint statistics component weights must be non-negative")
    ...
    flow_physics_w = float(flow_physics_w)
    if flow_physics_w < 0.0:
        raise ValueError("flow_physics_w must be non-negative")
    ...
    if flow_repa_aligner is not None and (
            flow_repa_w > 0.0 or flow_repa_structure_w > 0.0):
    ...
    for flow_step in range(flow_steps):
        ...
        active_flow_repa_structure_w = float(flow_repa_structure_w)
        if flow_repa_steps > 0 and flow_step >= int(flow_repa_steps):
        ...
        for _micro in range(flow_accum_steps):
            ...
            if flow_cache is not None:
                ...
                image_features = (
                    ...
                    if (flow_feature_align_w > 0.0 or active_flow_repa_w > 0.0
                ...
                image_feature_tokens = (
                    ...
                    if (
                        ...
                        or (
                            active_flow_repa_w > 0.0
                            and (flow_repa_mode in ("token", "both")
                                 or (flow_repa_mode == "auto"
                                     and has_image_embedding_sequences))
                        )
                    )
                    else None
                )
            ...
            with amp_autocast(device, train_precision):
                loss, parts = latent_flow_losses(
                    ...
                    endpoint_stats_w=flow_endpoint_stats_w,
                    ...
                    repa_structure_w=active_flow_repa_structure_w,
    ...
    report.update({
        ...
        **image_bucket_report,
        ...
        "flow_repa_structure_active_steps": (
            ...
            else min(int(flow_steps), int(flow_repa_steps))
            if flow_repa_structure_w > 0.0 else 0
        ),
        "flow_repa_steps": int(flow_repa_steps),
...
def selftest():
    ...
    stats_pred = torch.randn(3, 4, 2, 2)
    stats_target = torch.randn(3, 4, 2, 2)
    stats_loss, stats_parts = flow_endpoint_statistics_loss(stats_pred, stats_target)
    assert stats_loss.ndim == 0
    assert "flow_endpoint_stats_mean_loss" in stats_parts
    assert "flow_endpoint_stats_std_loss" in stats_parts
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for i in range(3):
            ...
            rows.append({
                ...
                "image_embedding_sequence": [
                    [1.0 if j == ((2 - i) % 3) else 0.0 for j in range(3)],
                    [0.5 if j == ((i + 1) % 3) else 0.0 for j in range(3)],
                    [0.25 if j == (i % 3) else 0.0 for j in range(3)],
                ],
            })
        ...
        same_physics, same_physics_parts = visual_physics_loss(
            blob, blob, prefix="unit_physics")
        shifted_physics, shifted_physics_parts = visual_physics_loss(
            shifted_blob, blob, prefix="unit_physics")
        assert float(same_physics) < 1.0e-6
        assert float(shifted_physics) > float(same_physics)
        assert "unit_physics_centroid_l1" in shifted_physics_parts
        ...
        ref_gate_pass = reference_reproduction_gate_report(
            ...
            max_texture_stats_l1=0.10, max_physics_l1=0.10,
            max_selected_score=1.0)
        ...
        assert math.isclose(report["flow_physics_w"], 0.01)
        ...
        assert "reference_flow_physics_l1" in reference_meta
...
def main(argv=None):
    ...
    ap.add_argument("--flow-repa-structure-w", type=float, default=0.0,
    ...
    ap.add_argument("--flow-endpoint-stats-w", type=float, default=0.0,
                    dest="flow_endpoint_stats_w",
                    help="batch latent endpoint mean/std matching loss weight")
    ap.add_argument("--flow-endpoint-stats-mean-w", type=float, default=1.0,
                    dest="flow_endpoint_stats_mean_w",
                    help="mean component weight inside --flow-endpoint-stats-w")
    ap.add_argument("--flow-endpoint-stats-std-w", type=float, default=1.0,
                    dest="flow_endpoint_stats_std_w",
                    help="std component weight inside --flow-endpoint-stats-w")
    ...
    ap.add_argument("--flow-spatial-relation-w", type=float, default=0.0,
                    ...
                    help=("local latent neighbor relation loss weight for preserving "
                          "spatial structure"))
    ap.add_argument("--flow-physics-w", type=float, default=0.0,
                    dest="flow_physics_w",
                    help=("generic latent visual mass/edge moment loss weight for "
    ...
    ap.add_argument("--sample-reference-max-physics-l1", type=float,
                    default=None, dest="sample_reference_max_physics_l1",
                    help=("fail after --sample-reference-grid-out if reference "
                          "visual mass/edge moment L1 is above this threshold"))
    ...
    if args.flow_endpoint_stats_w < 0.0:
        ap.error("--flow-endpoint-stats-w must be non-negative")
    if args.flow_endpoint_stats_mean_w < 0.0 or args.flow_endpoint_stats_std_w < 0.0:
    ...
    if args.flow_physics_w < 0.0:
        ap.error("--flow-physics-w must be non-negative")
    ...
    (ae, flow, conditioner, prompt_vocab, text_aligner, report,
     flow_ema, conditioner_ema) = train_latent_flow(
        ...
        flow_endpoint_stats_w=args.flow_endpoint_stats_w,
        ...
        flow_physics_w=args.flow_physics_w,
    ...
    torch.save({
        ...
        "flow_spatial_relation_w": report.get(
            "flow_spatial_relation_w", args.flow_spatial_relation_w),
        "flow_physics_w": report.get("flow_physics_w", args.flow_physics_w),
