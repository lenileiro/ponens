...
reasoning core (recurrence/IDF + sentence relevance) powers four capabilities:
  ...
  prune(question, passage, tau)  -> kept context  (drop question-irrelevant sentences; the Provence task)
...
def prune(question, passage, tau=0.5):
    """RAG context pruning: keep the sentences relevant to `question`, drop the rest (the Provence task)."""
...
def selftest():
    ...
    kept = prune("How tall is the tower?", passage, tau=0.5)
    print("prune    -> ", repr(kept[:90]))
    assert "330" in kept and len(kept) < len(passage)

    print("runtime_api selftest OK (classify / answer / summarize / prune -- all zero-training runtime inference)")
    return 0
