...
def text_reading_cmd(args, py):
    ...
    cmd = (
        ...
        f"--reading-source-balance-w {args.reading_source_balance_w} "
...
def apply_image_quality_preset(args):
    ...
    args.image_flow_reference_condition_p = max(
        float(args.image_flow_reference_condition_p), 0.25 if hq else 0.1)
    ...
    args.image_flow_physics_w = max(
        float(args.image_flow_physics_w), 0.03 if hq else 0.01)
    ...
    if (args.vision_understanding or args.image_latent or args.image_embed
        ...
        if args.image_latent:
            ...
            if args.image_sample_reference_grid:
                ...
                for flag, value in (
                        ...
                        ("sample-reference-max-physics-l1",
                         args.image_sample_reference_max_physics_l1),
            ...
            train = (f"{IL} --train --ae-steps {args.train_steps or 800} "
                     ...
                     f"--flow-physics-w {args.image_flow_physics_w} "
                     ...
                     f"--flow-reference-condition-p "
                     f"{args.image_flow_reference_condition_p} "
...
def main():
    ...
    ap.add_argument("--reading-source-balance-w", type=float,
                    ...
                    dest="reading_source_balance_w",
                    help=("smooth source-balanced raw-reading sampling passed "
                          "to thinking.text; 0 disables source balancing"))
    ...
    ap.add_argument("--image-flow-reference-condition-p", type=float, default=0.0,
                    dest="image_flow_reference_condition_p",
                    help=("probability of clean reference-latent conditioning during "
                          "image flow training"))
    ...
    ap.add_argument("--image-sample-reference-max-texture-stats-l1", type=float,
                    ...
                    help=("fail image reference reproduction if texture-statistics "
                          "L1 exceeds this"))
    ap.add_argument("--image-sample-reference-max-physics-l1", type=float,
                    default=None,
                    dest="image_sample_reference_max_physics_l1",
                    help=("fail image reference reproduction if visual mass/edge "
                          "moment L1 exceeds this"))
    ...
    ap.add_argument("--image-flow-physics-w", type=float, default=0.0,
                    dest="image_flow_physics_w",
                    help="generic latent visual mass/edge moment loss weight")
    ...
    if args.text_reading:
        ...
        if args.reading_source_balance_w < 0.0:
            sys.exit("ERROR: --reading-source-balance-w must be non-negative")
    ...
    if args.image_flow_physics_w < 0.0:
        sys.exit("ERROR: --image-flow-physics-w must be non-negative")
    ...
    if (args.image_flow_reference_condition_p < 0.0
            or args.image_flow_reference_condition_p > 1.0):
        sys.exit("ERROR: --image-flow-reference-condition-p must be in [0, 1]")
    if args.image_flow_reference_condition_p > 0.0 and not args.image_flow_self_condition:
        sys.exit("ERROR: --image-flow-reference-condition-p requires --image-flow-self-condition")
