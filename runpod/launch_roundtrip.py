...
def main():
    ...
    ap.add_argument("--name", default="roundtrip-probe")
    ...
    remote_cmd = (
        f"cd {REMOTE} && rm -f roundtrip.log /root/roundtrip.log && "
        ...
        f"2>&1 | tee /root/roundtrip.log; "
        f"cp /root/roundtrip.log {REMOTE}/roundtrip.log 2>/dev/null; true")

    print("=== PLAN === ROUND-TRIP read->comprehend->write->verify (thinking/roundtrip.py) on H100, brain-verified")
    print(f"gpu/cloud : {args.gpu} / {args.cloud}  (disk {args.disk}GB, name {args.name})")
    print(f"sweep     : {len(SWEEP)} configs")
    for (tag, pp, d, steps) in SWEEP:
        print(f"   [{tag}] per_pos={pp} (~{pp*4} concepts) d{d} steps{steps}")
    print(f"deploy    : working-dir -> pod:{REMOTE} (uncommitted ok); WordNet installed on pod")
    print(f"fetch     : roundtrip.log + runs/meaning_*.json -> {HERE}/")
    ...
    try:
        ...
        sh(f"{ssh} 'rm -f /root/DONE /root/roundtrip.log /root/launch.out && "
        ...
        while time.time() - t1 < cap:
            ...
            try:
                r = subprocess.run(f"{ssh} 'test -f /root/DONE && echo DONE; "
                                   f"tail -1 /root/roundtrip.log 2>/dev/null'",
