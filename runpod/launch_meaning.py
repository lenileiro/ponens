...
def build_sweep_cmd():
    lines = []
    for (tag, pp, d, steps, batch) in SWEEP:
        lines.append(
            f"echo '=== CONFIG {tag}: per_pos={pp} d{d} steps{steps} batch{batch} isa={ISA_MODE} ==='; "
            f"python -m thinking.meaning --per-pos {pp} --d {d} --steps {steps} --batch {batch} "
            f"--isa-mode {ISA_MODE} --device cuda --out runs/meaning_{tag}.json || true")
    return " ; ".join(lines)
...
def main():
    ...
    ap.add_argument("--name", default="meaning-copy-probe")
    ...
    preflight = (
        ...
        "print('wordnet ok', len(list(wn.all_synsets())), 'synsets')\"")
    remote_cmd = (
        f"cd {REMOTE} && rm -f meaning.log /root/meaning.log && "
        ...
        f"2>&1 | tee /root/meaning.log; "
        f"cp /root/meaning.log {REMOTE}/meaning.log 2>/dev/null; true")
    ...
    for (tag, pp, d, steps, batch) in SWEEP:
        print(f"   [{tag}] per_pos={pp} (~{pp*4} concepts) d{d} steps{steps} batch{batch}")
    print(f"deploy    : working-dir -> pod:{REMOTE} (uncommitted ok); WordNet installed on pod")
    print(f"fetch     : meaning.log + runs/meaning_*.json -> {HERE}/")
    ...
    try:
        ...
        sh(f"{ssh} 'rm -f /root/DONE /root/meaning.log /root/launch.out && "
        ...
        while time.time() - t1 < cap:
            ...
            try:
                r = subprocess.run(f"{ssh} 'test -f /root/DONE && echo DONE; "
                                   f"tail -1 /root/meaning.log 2>/dev/null'",
            ...
            if "DONE" in (r.stdout or ""):
                print("run complete")
                break
        tags = " ".join(f"runs/meaning_{t}.json" for (t, *_rest) in SWEEP)
        fetch = (f"{ssh} 'cd {REMOTE} && tar czf - meaning.log {tags} 2>/dev/null' "
                 f"| tar xzf - -C {HERE}")
        sh(fetch)
